...
(function () {
  ...
  function draw() {
    var lightMode = document.documentElement.getAttribute('data-theme') === 'light';
    var TXT = lightMode ? '#1c2030' : '#e8eaf0';
    var MUT = lightMode ? 'rgba(93,100,116,0.95)' : 'rgba(139,144,160,0.95)';
    ...
    nodes.forEach(function (n) {
      if (n.type === 'a') {
        ...
        ctx.fillStyle = n === hover ? TXT : col(n.d.color, lightMode ? 0.8 : 0.95);
...
<html lang="ko" data-lang="ko" data-theme="dark">
<head>
<meta charset="UTF-8">
<script>
try {{
  var _t = localStorage.getItem('portfolio-theme')
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  html[data-theme="light"] .lang-toggle {{ background: rgba(255,255,255,.92); box-shadow: 0 6px 20px rgba(28,32,48,.12); }}
  .theme-btn {{ font-size: .85rem; padding: 5px 9px; }}
  .ti-light {{ display: none; }}
  html[data-theme="light"] .ti-dark {{ display: none; }}
  html[data-theme="light"] .ti-light {{ display: inline; }}
  /* 라이트 모드 보정: 어두운 배경 전제의 그림자·틴트 완화 */
  html[data-theme="light"] header.hero {{
    background:
      radial-gradient(800px 400px at 50% -10%, rgba(91,141,239,.12), transparent),
      radial-gradient(600px 300px at 80% 0%, rgba(167,139,250,.09), transparent);
  }}
  html[data-theme="light"] .exhibit:hover {{ box-shadow: 0 18px 50px rgba(28,32,48,.12); }}
  html[data-theme="light"] .ex-shots .shot {{ box-shadow: 0 14px 36px rgba(28,32,48,.18); }}
  html[data-theme="light"] .ex-poster .ex-icon {{ box-shadow: 0 18px 44px rgba(28,32,48,.25); }}
  html[data-theme="light"] .ex-head-icon {{ box-shadow: 0 4px 14px rgba(28,32,48,.15); }}
  html[data-theme="light"] .poster-price {{ background: rgba(0,0,0,.05); }}
  html[data-theme="light"] .rating {{ background: rgba(176,125,0,.10); border-color: rgba(176,125,0,.3); color: #8a6a00; }}
  html[data-theme="light"] .stars {{ color: #c99700; }}
  html[data-theme="light"] .rating-num {{ color: #8a6a00; }}
  html[data-theme="light"] .rating-count {{ color: rgba(138,106,0,.75); }}
  html[data-theme="light"] .pg-tip {{ background: rgba(255,255,255,.97); box-shadow: 0 8px 24px rgba(28,32,48,.14); }}
  html[data-theme="light"] .story-problem .story-tag {{ color: #c2483c; }}
  html[data-theme="light"] .story-solution .story-tag {{ color: #1e8e6a; }}
  html[data-theme="light"] .highlight {{ color: #7c5fd3; }}
  .lang-btn {{
...
<body>
  <div class="lang-toggle" role="group" aria-label="언어 · 테마 / Language · Theme">
    ...
    <button type="button" class="lang-btn theme-btn" id="themeBtn" aria-label="테마 전환 / Toggle theme"><span class="ti-dark">☀️</span><span class="ti-light">🌙</span></button>
  </div>
  <header class="hero">
    <div class="wrap">
  ...
  (function () {{
    ...
    apply(lang);
    var btns = document.querySelectorAll('.lang-btn[data-set-lang]');
