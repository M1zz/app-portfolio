...
struct AppsGridView: View {
    ...
    private var filteredApps: [AppModel] {
        ...
        // Sort by: 1) localProjectPath presence, 2) completion percentage, 3) version, 4) name
        return apps.sorted { app1, app2 in
            let hasPath1 = app1.localProjectPath != nil && !app1.localProjectPath!.isEmpty
            let hasPath2 = app2.localProjectPath != nil && !app2.localProjectPath!.isEmpty

            if hasPath1 != hasPath2 {
                return hasPath1  // Apps with path come first
            }

            let completion1 = calculateProjectCompletion(for: app1)
            let completion2 = calculateProjectCompletion(for: app2)

            if completion1 != completion2 {
                return completion1 > completion2  // Higher completion first
            }

            // Version comparison (higher version first)
            let versionCompareResult = compareVersions(app1.currentVersion, app2.currentVersion)
            if versionCompareResult != 0 {
                return versionCompareResult > 0  // Higher version first
            }
