...
for f in swift_files:
    basename = os.path.basename(f)
    ref_section += f'\t\t{file_uuids[f]} /* {basename} */ = {{isa = PBXFileReference; fileEncoding = 4; lastKnownFileType = sourcecode.swift; path = "{f}"; sourceTree = "<group>"; }};\n'
ref_section += f'\t\t{assets_uuid} /* Assets.xcassets */ = {{isa = PBXFileReference; lastKnownFileType = folder.assetcatalog; path = Resources/Assets.xcassets; sourceTree = "<group>"; }};\n'
...
# Main group children - all files flat
children = []
for f in swift_files:
    basename = os.path.basename(f)
...
print(f"✅ Xcode 프로젝트 생성 완료 ({len(swift_files)}개 파일 포함)")
