# TODO — 생활도구점 페이지 (2026-09-24 ~ 09-26)

## 목표
낯선 앱들을 '만들게 된 이야기 + 구매 링크'로 묶어 보여 주는 다이소형 가게 페이지
배포: https://m1zz.github.io/app-portfolio/shop.html

## 작업
- [x] 뉴스레터 글 2편 내용 수집 (p/187 클립키보드, p/197 무지개)
- [x] `docs/shop.html` 작성 (한/영, 라이트/다크, 모바일 대응)
- [x] 배포 워크플로우 트리거에 `docs/shop.html` 추가
- [x] 구조 개편: 도구 그리드 + 도구별 상세 화면(#id), ITEMS 배열로 데이터화 (40개 대비)
- [x] 매장 용어(코너·진열) 제거, 카드 버튼 '만든 이유 보기'
- [x] 앱 내 구입(iap) 표시, 욕망의 무지개 '무료부터'
- [x] 카드 문구를 읽는 사람의 장면(2인칭)으로, 분류를 '불편한 순간'으로
- [x] 상세 화면 '내 경우엔 얼마나?' 계산기
- [x] 커밋 & push
- [ ] **사용자 자료 필요**: 사용 장면 10초 영상 (상세 화면에 삽입)
- [ ] **사용자 자료 필요**: 실제 리뷰 한두 줄 (클립키보드 평점 4.7)
- [ ] 클립키보드 · 무지개 공방 앱 내 구입 여부 확인
- [ ] 쇼케이스(index.html, 빌드 스크립트)에서 가게로 가는 링크 추가 여부 결정
- [ ] 다음 글 발행 시 도구 추가 (큰 시간의 공백 만들기)
- [ ] 로컬 main 정리: 원격과 갈라짐, bf5b18a(무지개 공방 이름 통일) 미반영
